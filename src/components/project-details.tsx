 ...
 */
export function ProjectDetails({ project, transactions, primaryCurrency, defaultExchangeRate }: ProjectDetailsProps) {
  ...
  const displayCurrency =
    project.currency ||
    transactions[0]?.accounts?.currency ||
    'RUB';
  ...
  const stats = transactions.reduce(
    (acc, transaction) => {
      ...
      // Если exchange_rate !== 1, то валюта транзакции отличается от валюты счета
      ...
      } else if (displayCurrency === accountCurrency) {
        // Случай 2: Валюта проекта совпадает с валютой счета
        // Используем converted_amount (сумма в валюте счета)
        amountInProjectCurrency = transaction.converted_amount || transaction.amount;
      } else if (displayCurrency === primaryCurrency) {
        // Случай 3: Валюта проекта совпадает с primary_currency
        // Используем converted_amount (уже в primary_currency, которая равна валюте счета)
        amountInProjectCurrency = transaction.converted_amount || transaction.amount;
      } else {
        // Случай 4: Валюта проекта отличается от валюты транзакции, валюты счета и primary_currency
        // Нужно конвертировать из primary_currency (валюта счета) в валюту проекта
        // Используем сохраненный project_exchange_rate, если он есть, иначе текущий курс по умолчанию
        
        // Сначала получаем сумму в primary_currency (валюта счета)
        const amountInPrimaryCurrency = transaction.converted_amount || transaction.amount;
        
        // Используем сохраненный курс к валюте проекта, если он есть
        // Это предотвращает пересчет старых транзакций при изменении курса по умолчанию
        if (transaction.project_exchange_rate !== null && transaction.project_exchange_rate !== undefined) {
          // Используем сохраненный курс к валюте проекта
          // project_exchange_rate - это курс от primary_currency к валюте проекта
          amountInProjectCurrency = amountInPrimaryCurrency * transaction.project_exchange_rate;
        } else {
          // Если сохраненного курса нет (старые транзакции), используем текущий курс по умолчанию
          // default_exchange_rate - это курс RUB/USD (сколько рублей за доллар)
          if (primaryCurrency === 'RUB' && displayCurrency === 'USD') {
            // Конвертируем RUB -> USD: делим на курс
            amountInProjectCurrency = amountInPrimaryCurrency / defaultExchangeRate;
          } else if (primaryCurrency === 'USD' && displayCurrency === 'RUB') {
            // Конвертируем USD -> RUB: умножаем на курс
            amountInProjectCurrency = amountInPrimaryCurrency * defaultExchangeRate;
          } else {
            // Если валюты не совпадают с ожидаемыми, используем converted_amount
            // (это не должно происходить, но на всякий случай)
            amountInProjectCurrency = amountInPrimaryCurrency;
          }
        }
      }
  ...
  const handleCreateTransaction = (type: 'income' | 'expense' | 'withdrawal') => {
                  ...
                  <p className="text-lg font-semibold">
                    ...
                    {project.counterparties.type && (
                      <span className="ml-2 text-sm font-normal text-zinc-500">
                        ({project.counterparties.type === 'client' ? 'Клиент' : 'Подрядчик'})
                      </span>
                    )}
                  </p>
                </div>
