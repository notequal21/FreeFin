...
- `transaction_date`: date — Фактическая дата транзакции. По умолчанию — текущая дата. Позволяет добавлять транзакции задним числом. При подтверждении запланированной транзакции (изменение `is_scheduled` с `true` на `false`) автоматически устанавливается текущая дата, если она не была установлена ранее.
- `created_at`: timestamp — Дата и время создания транзакции в системе.
